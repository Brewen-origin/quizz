      ...
      {isFinished && players.length >= 1 && (
        <div className="flex items-end justify-center gap-2 mb-8 h-32">
          ...
          <div className="flex flex-col items-center flex-1">
            ...
            <p className="text-xs text-yellow-300 font-bold truncate w-full text-center">
              {players[0].pseudo}
            </p>
      ...
      <div className="bg-gray-900 rounded-2xl overflow-hidden mb-6">
        {players.map((player, index) => {
          ...
          return (
            <div
              key={player.id}
              className={`flex items-center gap-3 px-4 py-3 border-b border-gray-800 last:border-0 ${isMe ? "bg-indigo-900/40" : ""}`}
            >
              <span className="text-lg w-8 text-center">
                {index < 3 ? MEDALS[index] : `${index + 1}.`}
              </span>
              <span
                className={`flex-1 font-medium ${isMe ? "text-indigo-300" : "text-white"}`}
              >
                {player.pseudo}
                {isMe && (
                  <span className="text-xs text-indigo-400 ml-2">(toi)</span>
                )}
                {player.is_host && (
